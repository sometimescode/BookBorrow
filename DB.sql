...
status ENUM('Checked Out', 'Available', 'Pending Check Out'),
...
requested_copy_id int,
PRIMARY KEY (id),
FOREIGN KEY(requester_id) references accounts(id),
FOREIGN KEY(requested_book_id) references book_entries(id),
FOREIGN KEY(requested_copy_id) references book_copies(id)
);
-- change checkout record id to book copy id to store book copy id that's required when a pending
-- request is approved. this will go into the calculation for available books
